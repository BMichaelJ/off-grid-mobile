...
> **2026-08-21 final status** (see [codex re-evaluation](../reports/2026-08-20-codex-reeval.md)): **Stage 2 is done except 2.8.** Pre-Stage-2 correctness landed first — box corner-clamping + `loadModel` promise-dedup (**PR #17**). Then: **2.1** PR #18 (`MiewIDModelRecord`, persist v1 migration, startup reconciliation, capture gate + compatibility filter) · **2.2** PR #22 (validator + quarantine + `setPacks` + bounds-checked `getEmbeddingsForIndividual`) · **2.3** PR #23 (staging + SHA-256 + atomic move + retry/backoff + `acquireMiewidModel`; hosting still open — URL/hash live in `src/config/modelSources.ts`) · **2.5** PR #24 (refcounted unload) · **2.6** PR #25 (partial-success `processPhoto`) · **2.7** PR #26 (ONNX I/O contract validation + NaN-proof matcher, covers the new codex finding #12) · **2.4** PR #27 (pack grouping by `{species, featureClass, detector, embeddingModelVersion}` + pack `embeddingModel.inputSize/normalize` honored, covers new codex finding #11). Remaining: **2.8 Settings UX** (first UI consumer of `acquireMiewidModel`), the **1.5 hosting decision**, then Stage 3 device verification.
